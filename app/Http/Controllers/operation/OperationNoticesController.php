...
use App\Jobs\operation\AwardNoticeStatus;
...
class OperationNoticesController extends Controller
{

    public function operationAwardNotices(Request $request, $option)
    {
        if ($option == 'upload') {
            return $this->operationFilesUpload($request);
        }
        ...
        if ($option == 'view') {
            ...
            return view('operations.notices.award-notice', compact('notices'));
        }
    ...
    public function operationFilesUpload(Request $request)
    {
    ...
    public function operationVacateNotices()
    {
        return view('operations.notices.vacate_notices');
    }

    public function operationNoticeOptions(Request $request)
    {
        ...
        if ($request->status_award == 1) {
            ...
            $pdf = PDF::loadView('operationsy.components.contract-template')->setPaper('legal', 'portrait');
            $dompdf = $pdf->getDomPDF();
            $canvas = $dompdf->getCanvas();
            $pdf->save(storage_path("app/{$directorypath}/{$pdfFileName}"));

            AwardNoticeStatus::dispatch($awardnotice);
        }
        return back()->with([
            'notice' => $awardnotice,
            'modal_open' => true,
    ...
    // public function operationAwardNotices()
    // {
    //     return view("operations.notices.award-notice");
    // }
