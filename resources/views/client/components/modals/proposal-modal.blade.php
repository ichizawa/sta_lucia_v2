<div class="modal fade" id="clientleaseProposal" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
...
<script>
    $(document).ready(function () {
        // $('.showclientProposalContents').click(function (e) {
        //     // CCJEDITRDC
        ...
        //     $.ajax({
        //         url: "{{ route('client.show.proposal') }}",
        //         type: "GET",
        //         data: {
        ...
        $.ajax({
            url: `/tenant/show-proposal/${proposal_id}`, // This directly hits the URL
            type: "GET",
            success: function (data) {
                if (data.pdf_url) {
                    $('#proposal-pdf').attr('src', data.pdf_url + '#zoom=175');
                ...
                $('#exampleModalLabel').text(data.proposal.company_name);

                // Display document status message
                if (data.document_status && data.document_status.status === 0) {
                    $('#exampleModalLabel').append(' <span class="badge bg-danger">Files still not uploaded!</span>');
                    $('#rejectNewProposal').attr('disabled', true);
                    $('#approveNewProposal').attr('disabled', true);
                } else {
                    $('#rejectNewProposal').attr('disabled', false);
                ...
                // Show or hide approve/reject buttons based on proposal status
                ...
                // Show the modal
                $('#clientleaseProposal').modal('show');
            },
            error: function (xhr, status, error) {
                console.error('An error occurred while fetching the proposal:', error);
            }
        });
    });
