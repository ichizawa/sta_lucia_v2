...
@section('content')
    @include('lease-admin.components.modals.view-tenant-files-modal')
                ...
                <div class="card card-round">
                    <div class="card-header">
                        <div class="d-flex align-items-center">
                            ...
                            <a href="{{ route('lease.add.tenants') }}" class="btn btn-sta ms-auto">
    ...
    <script>
        $(document).ready(function (e) {
            $('.view_documents').click(function () {
                ...
                $.ajax({
                    url: "{{ route('lease.tenant.documents') }}",
                    type: "GET",
                    data: {
                        owner_id: owner_id
                    },
                    ...
                    success: function (data) {
                        ...
                        $('.approveTenantDocuments').click(function () {
                            var owner_id = $(this).data('owner-id');
                            $.ajax({
                                url: "{{ route('lease.tenant.documents.approve') }}",
                                type: "POST",
                                data: {
                                    ownerid: owner_id,
                                },
                                ...
                                $('#tenant-documents-file').on('change',
                                    function () {
                                        ...
                                        formData.append('tenant_doc_file',
                                            e);
                                        $.ajax({
                                            url: "{{ route('lease.submit.documents') }}",
                                            type: "POST",
                                            processData: false,
                                            contentType: false,
                                            data: formData,
        ...
        function delete_tenant_func(tenant_id) {
            swal({
                ...
                if (willDelete) {
                    $.ajax({
                        url: "{{ route('lease.delete.tenants') }}",
                        type: 'POST',
                        data: {
                            id: tenant_id
                        },
