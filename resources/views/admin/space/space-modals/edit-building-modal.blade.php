<div class="modal fade" id="editBuildingModal" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
  <div class="modal-dialog modal-xl">
    <div class="modal-content">
      <form id="modalEditBuilding" action="{{route('space.option.space', 'submitBuilding')}}" method="POST" enctype="multipart/form-data">
        @csrf
                ...
                <div id="mallListCodes" class="container-fluid">
                  <div class="row">
                    ...
                    <div class="col-md-12">
                      ...
                      <select id="edit-building-search-mall-code" class="form-control" placeholder="Select Mall Code..." name="mallCode"
                        disabled>
                    ...
                    <div class="col-md-12">
                      ...
                      <input type="text" id="editBuildingID" class="form-control" name="bldg_number" />
                      <input type="text" id="bldgid" class="form-control" name="bldgid" hidden />
                    </div>
                    <div class="col-md-12">
                      <label>Total Area of Building</label>
                      <input type="text" class="form-control" id="total_area" name="total_area" placeholder="Total Area" readonly />
                    </div>
                    <div class="col-md-12">
                      <label>Total Available Area</label>
                      <input type="text" class="form-control" id="total_available" name="total_available"
                        placeholder="Total Available Area" readonly />
                    </div>
                    <div class="col-md-12">
                      <label>Total Leased Area</label>
                      <input type="text" class="form-control" id="total_leased" name="total_leased" placeholder="Total Leased Area"
                        readonly />
                    </div>
                  </div>
                </div>
                ...
                <div class="container-fluid">
                  <div class="row">
                    <div class="col-md-12">
                      ...
                      <input type="file" id="image-upload2" class="form-control" name="bldg_image" />
                    </div>
                    <div class="col-md-12 mt-3">
                      <img
                        ...
                        class="img-fluid card-img" id="show-image2" alt="..." style="height: 55vh; object-fit: cover" />
                    </div>
                  </div>
                </div>
              </div>
...
<script>
  $(document).ready(function() {
    ...
    $('.editBuilding').click(function() {
      ...
      $.ajax({
        url: "{{route('space.edit.building', 'editBuilding')}}",
        type: "GET",
        data: {
        ...
        success: function(data) {
          $.each(data, function(key, value) {
            // console.log(data);
            $('#edit-building-search-mall-code').val(value.mallcodes.id);
            $('#editBuildingID').val(value.bldgnum);
            $('#bldgid').val(value.id);
            $('#total_area').val(data[0].mallcodes.total_area);
            $('#total_available').val(data[0].mallcodes.total_available);
            $('#total_leased').val(data[0].mallcodes.total_leased);
            var img = data[0].bldgimage ? `{{ asset('storage/bldg_image') }}/${data[0].bldgimage}` : 'https://images.pexels.com/photos/28216688/pexels-photo-28216688/free-photo-of-autumn-camping.png?auto=compress&cs=tinysrgb&w=1260&h=750&dpr=1';
