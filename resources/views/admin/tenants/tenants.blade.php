                                    ...
                                    @foreach ($owner->companies as $company)
                                        @foreach ($owner->representatives as $rep)
                                            ...
                                            <tr>
                                                ...
                                                <td>
                                                    {!! $rep->status ? '<span class="badge bg-success">Active</span>' : '<span class="badge bg-warning">Pending</span>' !!}
                                                </td>
                                                <td>
                                                    {!! $owner->doc_status ? '<span class="badge bg-success">Approved</span>' : '<span class="badge bg-warning">Pending</span>' !!}
                                                </td>
                                                <td>
                ...
                success: function (data) {
                    ...
                    $.each(data[0].documents, function (key, value) {
                        if (tenant_type == 'Individual') {
                            if (key === 'sec_reg' || key === 'valid_idSig1' || key === 'valid_idSig2') {
                                return true;
                            }
                        } else {
                            if (key === 'dti_reg' || key === 'valid_id1' || key === 'valid_id2') {
                                return true;
                            }
                        }
                        if (value == null) {
                    ...
                    $('.approveTenantDocuments').click(function () {
                        ...
                        $.ajax({
                            ...
                            success: function (data) {
                                //    console.log(data);
                                $('#tenant-show-table').find('td').eq(7).html('<span class="badge bg-success">Approved</span>');
                    ...
                    $.each(data, function (key, value) {
                        ...
                        $.each(value.documents, function (key, value) {
                            ...
                            if (value == null) {
                                documentStatus = '<span class="badge bg-warning">Not Uploaded</span>';
                            } else {
                                documentStatus = '<span class="badge bg-success">Uploaded</span>';
                            }
                    ...
                    $('.viewDocument').click(function () {
                        ...
                        $('#upload-File').click(function () {
                            ...
                            var tenant_doc_id = $('#tenant-documents-id').val();
                            var tenant_doc_id2 = $('#tenant-documents-id2').val();
                            var owner_id = $('#tenant-documents-owner-id').val();
                            $('#tenant-documents-file').on('change', function () {
                                var e = this.files[0];
                                var formData = new FormData();
                                formData.append('tenant_doc_id', tenant_doc_id);
                                formData.append('tenant_doc_id2', tenant_doc_id2);
                                formData.append('tenant_doc_owner_id', owner_id);
                                formData.append('tenant_doc_file', e);
                                $.ajax({
